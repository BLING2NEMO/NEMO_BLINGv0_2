...
!
! History: This model is ported exactly from GFLD model to NEMO.
! CAUTION. GFDL uses units of mol/kg while NEMO mol/m3.
! MClaret@McGill@04/2016
!
!--------------------------------------------------------------------
...
subroutine FMS_ocmip2_co2calc(isd,ied,jsd,jed,isc,iec,jsc,jec, mask,            &
...
subroutine FMS_ocmip2_co2_alpha(isd,jsd,isc,iec,jsc,jec, t, s, mask, alpha, scale)  !{
...
!
!type(CO2_dope_vector), intent(in)       :: dope_vec
integer, intent(in) :: isd, jsd, isc, iec, jsc, jec

!real, dimension(isc:,jsc:), intent(in)  :: t
!real, dimension(isc:,jsc:), intent(in)  :: s
!real, dimension(isc:,jsc:), intent(in)  :: mask
